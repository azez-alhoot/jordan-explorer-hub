    ...
    <section id="why-us" className="py-24 bg-primary text-primary-foreground relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          ...
          <p className={`font-body text-lg text-sand/80 max-w-2xl mx-auto ${isRTL ? 'leading-loose' : ''}`}>
            {t('why.description')}
          </p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <motion.div
              ...
              className={`group p-8 rounded-2xl bg-primary-foreground/5 border border-primary-foreground/10 hover:bg-primary-foreground/10 transition-all duration-300 ${isRTL ? 'text-right' : ''}`}
            >
              <div className={`w-14 h-14 rounded-xl bg-accent/20 flex items-center justify-center mb-6 group-hover:bg-accent/30 transition-colors ${isRTL ? 'mr-0 ml-auto' : ''}`}>
                <feature.icon className="w-7 h-7 text-accent" />
              </div>
              <h3 className="font-display text-xl text-sand-light mb-3">{t(feature.titleKey)}</h3>
              <p className={`font-body text-sand/70 leading-relaxed ${isRTL ? 'leading-loose' : ''}`}>{t(feature.descKey)}</p>
            </motion.div>
          ))}
        </div>
      </div>
